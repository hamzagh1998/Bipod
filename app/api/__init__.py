...
from app.api.schemas import (
    ...
    StudioImageResponse,
    StudioPromptImproveRequest,
    StudioPromptImproveResponse,
)
...
async def get_system_config():
    ...
    try:
        ...
        async with httpx.AsyncClient(timeout=3.0) as client:
            ...
            imagine_models = (
                r_models.json().get("models", []) if r_models.status_code == 200 else []
            )
...
async def get_messages(
    conv_id: str, user_id: int = Depends(auth_service.get_current_user)
):
...
async def unlock_conversation(
    ...
    is_valid = await memory_service.verify_archive_password(
        conv_id, user_id, unlock.password
    )
    if not is_valid:
...
async def chat(
    request: ChatRequest, user_id: int = Depends(auth_service.get_current_user)
):
...
async def chat_stream(
    ...
    async def run_chat() -> None:
        try:
            ...
            await queue.put(
                {"type": "error", "detail": exc.detail, "status_code": exc.status_code}
            )
...
async def clear_memory(
    ...
    return {
        "status": "success",
        "message": f"Bipod has forgotten the recent past in {conversation_id}.",
    }


@router.post("/generate")
async def generate_image(
    payload: dict = Body(...), user_id: int = Depends(auth_service.get_current_user)
):
    ...
    try:
        ...
        return JSONResponse(
            status_code=resp.status_code, content=jsonable_encoder(content)
        )
    except HTTPException:
        raise
    except Exception as e:
        raise HTTPException(
            status_code=502, detail=f"Imagine service unavailable: {str(e)}"
        )
...
async def upscale_image(
    ...
    try:
        ...
        async with httpx.AsyncClient(timeout=1200.0) as client:
            resp = await client.post(
                f"{settings.IMAGINE_API_URL}/upscale", json=payload
            )
...
@router.get(
    "/studio/projects/{project_id}/images", response_model=List[StudioImageResponse]
)
...
async def improve_studio_prompt(
    ...
    try:
        ...
        raise HTTPException(
            status_code=502, detail=f"Prompt improvement failed: {str(e)}"
        )
